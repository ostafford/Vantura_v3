...
API token and any secrets are encrypted client-side (`src/lib/crypto.ts`: PBKDF2-SHA256 → AES-GCM 256-bit) using a key derived from the user's passphrase; the passphrase itself is never persisted. Biometric unlock (`src/lib/webauthn.ts`, `src/lib/biometricSession.ts`) is an optional convenience layer on top of the passphrase, not a replacement. See `SECURITY.md` and `Arch_Docs/08_Security.md` for the full model before touching auth/encryption code.
...
- `Arch_Docs/` — architecture/design reference (schema, calculation logic, sync strategy, security, UI/UX) — check the relevant doc before making non-trivial changes in that area, and update it if the change affects the documented design.
