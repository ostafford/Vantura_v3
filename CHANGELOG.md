...
- **Vantura can find recurring charges in your transaction history.** A new "Find recurring charges" button in the Upcoming section scans your settled transactions for steady-amount, steady-interval patterns (Netflix, gym, insurance…) and offers them as upcoming charges — one tap to add, or "Edit & add" to adjust first. Nothing is added automatically; a "Not recurring" dismissal is remembered so it's never re-offered. An accepted suggestion links itself for auto-clear-when-paid where it can. New `dismissed_charge_suggestions` table (schema v46), `docs/adr/0016`.
- **Savers keep a goal history, and interest earned is tracked per goal cycle.** A saver's goal is no longer a single overwritable number — reaching one records it and lets you set the next, with past goals (and the date each was reached) listed on the card. Each card also shows interest earned "since the current goal started" and all-time, and the goal-history list breaks interest down per cycle. Interest is detected from Up's `transaction_type = 'Interest'`. New `saver_goal_history` table (schema v44; `saver_goal_date_*` settings retired), profile export payload v6, `docs/adr/0015`.
