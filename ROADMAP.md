...
| Jun 21 | **Hosting migrated to Netlify** (`v0.5.2`) — All routes return HTTP 200 for every user on every device (PWA and browser); eliminates the GitHub Pages 404-on-sub-route issue permanently at the CDN level with no SW dependency. Base path simplified from `/Vantura_v3/` to `/`. GitHub Actions retained for CI quality gates; Netlify auto-deploys on push to `main`. Live at `https://vanturaup.netlify.app`. See `netlify.toml`, `vite.config.ts`, `.github/workflows/deploy.yml` |
| Jun 21 | **Toast slide animation** (`v0.5.2`) — Replaced Bootstrap's fade Toast with a custom CSS `@keyframes` component; notifications slide down from top on entry and slide back up on dismiss; applies to all toast calls app-wide. See `src/components/ToastProvider.tsx` |
| Jun 21 | **What's New modal empty-state guard** (`v0.5.2`) — Modal suppresses itself and silently marks version seen when no milestone entries exist for the current version, preventing a blank modal after patch-only version bumps. See `src/components/WhatsNewModal.tsx` |
