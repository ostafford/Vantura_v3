...
type SaverMonthlyFlowChartProps = {
  data: SaverMonthlyFlowPoint[]
  className?: string
  style?: React.CSSProperties
  'aria-label'?: string
}

export function SaverMonthlyFlowChart({
  data,
  className,
  style,
  'aria-label': ariaLabel,
}: SaverMonthlyFlowChartProps) {
  ...
  useEffect(() => {
    ...
    const maxVal = Math.max(...data.map((d) => Math.abs(d.flowCents)), 1)
    const labels = data.map((d) => d.monthLabel)

    const left = estimateLeftAxisValueLabelSpace(maxVal / 100, 11)
    const bottom = estimateBottomAxisLabelSpace(labels, 11, { rotatedDeg: -45 })
    ...
    const showTooltip = (point: SaverMonthlyFlowPoint, event: MouseEvent) => {
      if (!tooltipEl || !container.parentElement) return
      const absVal = Math.abs(point.flowCents)
      const flowLabel =
        absVal === 0
          ? 'No activity'
          : point.flowCents <= 0
            ? `Saved: $${formatMoney(absVal)}`
            : `Withdrew: $${formatMoney(absVal)}`
      const txLabel =
        point.txCount > 0
      ...
      setTooltipContent(
        tooltipEl,
        point.monthLabel,
        [flowLabel, txLabel].filter(Boolean)
      )
      tooltipEl.style.display = 'block'
      positionTooltip(tooltipEl, container, event, 150, 44)
    }
    ...
    // Bars for months with activity
    data.forEach((point) => {
      const absVal = Math.abs(point.flowCents)
      if (absVal === 0) return
      const barY = yScale(absVal)
      const barHeight = innerHeight - barY
      const color = point.flowCents <= 0 ? SAVE_COLOR : WITHDRAW_COLOR

      g.append('rect')
        .attr('x', xScale(point.monthLabel) ?? 0)
        .attr('y', barY)
    ...
    // Invisible hit targets for zero months so tooltip still works on hover
    data.forEach((point) => {
      if (Math.abs(point.flowCents) > 0) return
      const x = xScale(point.monthLabel)
      ...
      g.append('rect')
        ...
        .on('mouseover', (event: MouseEvent) => showTooltip(point, event))
